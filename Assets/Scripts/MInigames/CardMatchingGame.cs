...
public class CardMatchingGame : MonoBehaviour
{
    ...
    // ==========================================
    /// <summary>
    /// 카드 배열 전체를 앞면으로 보여주었다가 지정한 시간 후 다시 뒷면으로 뒤집습니다.
    /// </summary>
    /// <param name="allCards">뒤집을 카드 배열 또는 리스트</param>
    /// <param name="previewDuration">유저에게 보여줄 시간 (초)</param>
    /// <param name="flipInterval">각 카드가 순차적으로 뒤집히는 간격 (0이면 동시)</param>
    public static IEnumerator RevealAllCardsSequence(List<CardUI> allCards, float previewDuration = 2.0f, float flipInterval = 0.05f)
    {
        if (allCards == null) yield break;

        // 1. 모든 카드를 순차적으로 앞면으로 뒤집기
        foreach (CardUI card in allCards)
        {
            if (card != null) card.FlipToFront();
        ...
        // 3. 모든 카드를 순차적으로 뒷면으로 뒤집기
        foreach (CardUI card in allCards)
        {
            if (card != null) card.FlipToBack();
            if (flipInterval > 0f) yield return new WaitForSeconds(flipInterval);
        }
    ...
    private void InitializeGame()
    {
        ...
        //게임시작전, 카드를 모두 뒤집어 일정시간동안 유저에게 보여주는 함수
        StartCoroutine(RevealAllCardsSequence(spawnedCards, 2.0f, 0.05f));
