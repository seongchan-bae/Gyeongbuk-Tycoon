...
public class CardMatchingGame : MonoBehaviour
{
    ...
    // ⭐ 인스펙터가 비어있을 경우 무작위 테스트용 15개 유적지 리스트 반환
    private List<CardData> GetActiveCardDataList()
    {
        ...
        {
            "첨성대", "불국사", "석굴암", "하회마을", "포석정",
            "동궁과 월지", "문경새재", "주왕산", "보경사", "독도",
            "성류굴", "부석사", "소수서원", "호미곶", "대릉원"
        };
    ...
    private void SpawnCard(CardData data, Transform parent)
    {
        ...
        if (card != null)
        {
            card.SetupCard(data.id, data.siteImage, data.siteName, this);
    ...
    private void GameOver(bool isSuccess, string message)
    {
        ...
        if (isSuccess)
        {
            ...
            if (gameManager != null)
            {
                gameManager.addUserKnowledgePoint(rewardKnowledgePoint);
            }
        }
    }
